...
function deserialize(inputCas, inputStream)
    ...
    -- Paragraphs
    local para_list = {}
    local para_counter = 1
    if results["paragraphs"] ~= null then
        for i, para in ipairs(results["paragraphs"]) do
            local para_obj = luajava.newInstance("de.tudarmstadt.ukp.dkpro.core.api.segmentation.type.Paragraph", inputCas)
            para_obj:setBegin(para["begin"])
            para_obj:setEnd(para["end"])
            para_obj:addToIndexes()
            para_list[para_counter] = para_obj
