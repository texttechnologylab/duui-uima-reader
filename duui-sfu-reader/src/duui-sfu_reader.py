...
from sfu_reader_utils import read_sfu_negation, parse_xml_file
from sfu_reader_utils import Paragraph, Token, Negation, Sentence
...
app = FastAPI(
    ...
    docs_url="/api",
    redoc_url=None,
    title="SFUNegationReader",
    description="SFUNegationReader for TTLab TextImager DUUI",
    version="0.1",
...
def get_documentation() -> TextImagerDocumentation:
    ...
    documentation = TextImagerDocumentation(
        annotator_name=f"SFUNegationReader-Version: {settings.sfu_version}",
        version=str(settings.sfu_version),
...
def process(request: DUUIRequest) -> DUUIResponse: # negations, paras, tokens, doc_name, " ".join(text)
    if not QUEUE.empty():
        ...
        content = parse_xml_file(current_doc[1])
...
if __name__ == "__main__":
    uvicorn.run("duui-sfu_reader:app", host="0.0.0.0", port=9714, workers=1)