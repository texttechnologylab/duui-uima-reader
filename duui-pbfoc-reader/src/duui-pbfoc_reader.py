...
from pbfoc_reader_utils import read_pbfoc_file, parse_pbfoc_file
from pbfoc_reader_utils import Token, Negation, Sentence, Pos
...
app = FastAPI(
    ...
    docs_url="/api",
    redoc_url=None,
    title="PBFOC-NegationReader",
    description="PBFOC(PennTreebank - Focus)-NegationReader for TTLab TextImager DUUI",
    version="0.1",
...
def get_documentation() -> TextImagerDocumentation:
    ...
    documentation = TextImagerDocumentation(
        annotator_name=f"PBFOC-NegationReader-Version: {settings.pbfoc_version}",
        version=str(settings.pbfoc_version),
...
def process(request: DUUIRequest) -> DUUIResponse: # negations, paras, tokens, doc_name, " ".join(text)
    if not QUEUE.empty():
        ...
        content = parse_pbfoc_file(current_doc[1])  # tuple[list[Sentence], list[Token], list[Pos], list[Negation], str]
...
if __name__ == "__main__":
    uvicorn.run("duui-pbfoc_reader:app", host="0.0.0.0", port=9714, workers=1)