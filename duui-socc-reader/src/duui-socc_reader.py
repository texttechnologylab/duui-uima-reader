...
from socc_utils import read_socc_negation, convert_tsv
from socc_utils import Paragraph, Token, Negation
...
app = FastAPI(
    ...
    docs_url="/api",
    redoc_url=None,
    title="SoccNegationReader",
    description="SoccNegationReader for TTLab TextImager DUUI",
    version="0.1",
...
def get_documentation() -> TextImagerDocumentation:
    ...
    documentation = TextImagerDocumentation(
        annotator_name=f"SoccNegationReader-Version: {settings.socc_version}",
        version=str(settings.socc_version),
...
async def init_se_reader(file: UploadFile = File(...)) -> InitResponse:
    if QUEUE.empty():
        ...
        docs = []
        res = read_socc_negation(buffer, target_folder_name="Negation_annotation")
        for key in res:
            docs.append(convert_tsv(key, res[key]))
...
def process(request: DUUIRequest) -> DUUIResponse: # negations, paras, tokens, doc_name, " ".join(text)
    if not QUEUE.empty():
        ...
        return DUUIResponse(doc_name=current_doc[3],
                            sofa_str=current_doc[4],
                            token=current_doc[2],
                            negations=current_doc[0],
                            paragraphs=current_doc[1],
                            accepted=True)
    else:
...
if __name__ == "__main__":
    uvicorn.run("duui-socc_reader:app", host="0.0.0.0", port=9714, workers=1)