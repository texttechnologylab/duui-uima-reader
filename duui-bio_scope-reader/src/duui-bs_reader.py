...
from bs_reader_utils import Token, Negation, Sentence
...
app = FastAPI(
    ...
    docs_url="/api",
    redoc_url=None,
    title="BioScope-NegationReader",
    description="BioScope-NegationReader for TTLab TextImager DUUI",
    version="0.1",
...
def get_documentation() -> TextImagerDocumentation:
    ...
    documentation = TextImagerDocumentation(
        annotator_name=f"BioScope-NegationReader-Version: {settings.bs_version}",
        version=str(settings.bs_version),
...
def process(request: DUUIRequest) -> DUUIResponse: # negations, paras, tokens, doc_name, " ".join(text)
    if not QUEUE.empty():
        ...
        content = parse_bs_file(current_doc[1])  # tuple[list[Sentence], list[Token], list[Lemma], list[Pos], list[Negation], str]
...
if __name__ == "__main__":
    uvicorn.run("duui-bs_reader:app", host="0.0.0.0", port=9714, workers=1)