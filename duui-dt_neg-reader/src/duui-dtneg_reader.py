...
from dtneg_reader_utils import read_dtneg_file, parse_dtneg_file
from dtneg_reader_utils import Token, Negation, Sentence
...
app = FastAPI(
    ...
    docs_url="/api",
    redoc_url=None,
    title="DTNeg-NegationReader",
    description="DeepTutor-NegationReader for TTLab TextImager DUUI",
    version="0.1",
...
def get_documentation() -> TextImagerDocumentation:
    ...
    documentation = TextImagerDocumentation(
        annotator_name=f"DeepTutor-NegationReader-Version: {settings.dtneg_version}",
        version=str(settings.dtneg_version),
...
def process(request: DUUIRequest) -> DUUIResponse: # negations, paras, tokens, doc_name, " ".join(text)
    if not QUEUE.empty():
        ...
        content = parse_dtneg_file(current_doc[1])  # tuple[list[Sentence], list[Token], list[Negation], str]
...
if __name__ == "__main__":
    uvicorn.run("duui-dtneg_reader:app", host="0.0.0.0", port=9714, workers=1)